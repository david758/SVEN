#Required globals: X, Y
svm <- function(linear,Y,lambda,w,out,usegpu) {
  ...
  obj_fun_linear <- function(w,Y,lambda,out) {
    ...
    out <- pmax(out,0)
    obj <- sum(out^2) / 2 + lambda * (t(w) %*% w)[1] / 2
    grad <- lambda * w - X %*% (out * Y)
    sv <- which(out[] > 0)
  ...
  line_search_linear <- function(w,d,out,Y,lambda,tol) {
    ...
    while (TRUE) {
      out2 <- out - t * (Y * Xd)
      sv <- which(out2[] > 0)
      # ***Cannot be done on gpu***
      g <- wd + t * dd - (t(out2 * Y)[,sv] %*% Xd[sv,])[1]
      h <- dd + (t(Xd[sv,]) %*% Xd[sv,])[1]
      # ***************************
      t <- t - g/h
      if (g^2/h < tol) {
        break
  ...
  #Requires: X, Y
  primal_svm_linear <- function(Y,lambda,w,out,usegpu) {
    d <- dim(X)[1]
    n <- dim(X)[2]
    ...
    while(TRUE) {
      ...
      if (opt.lin_cg) {
        print("Sparse matrix not supported")
        #[step, foo, relres] = minres(@hess_vect_mult, -grad,...
        #                             opt.cg_prec,opt.cg_it,[],[],[],sv,lambda);
      }
      else {
        #Avoid copying matrix as much as possible
        if (length(sv) == n) {
          Xsv <- X
        }
        else if (usegpu) {
        ...
        #Cannot use pcg unless we rewrite function
    ...
    return(list(out_l=out_l,obj=objFunOut[["obj"]],w_l=w_l,alpha=alpha))
  }
  ...
  primal_svm_linear_cg <- function(Y,lambda,w,out) {
    ...
    while (TRUE) {
      ...
      if (t * (t(s) %*% go)[1] < opt.prec * obj) {
        stop <- 1
        break
      }
      
      be <- ((t(gn) %*% gn)[1] - 0 * (t(gn) %*% go)[1]) / (t(go) %*% go)[1]
      s <- be * s + gn
      go <- gn
    }
  ...
  line_search_nonlinear <- function(step,Kb,b,Y,lambda,fullstep) {
    ...
    while (TRUE) {
      ...
      g <- lambda * (Kbs + t * Kss) - (t(Ks[sv,]) %*% (Y[sv] * out[sv,]))[1]
  ...
  primal_svm_nonlinear <- function(Y,lambda,usegpu) {
      training <- which(Y[] != 0)
      n <- length(training)
      if (n >= 1100) {
        perm <- sample(1:n,n)
        ...
        beta <- primal_svm_nonlinear(Y2,lambda,usegpu)
        sv <- which(beta[] != 0)
        if (usegpu) {
          Kb <- gpuMatrix(K[training,sv]) %*% beta[sv,]
        }
        else {
        ...
        sv <- training
        if (usegpu) {
          beta <- gpuMatrix(0,nrow=dim(Y)[1],ncol=1)
          Kb <- gpuMatrix(0,nrow=n,ncol=1)
        }
        else {
          beta <- matrix(0,nrow=dim(Y)[1],ncol=1)
          Kb <- matrix(0,nrow=n,ncol=1)
        }
      }
      ...
      while (TRUE) {
        ...
        out <- 1 - Y[training,] * Kb
        sv <- training[out[] > 0]
        obj <- lambda * t(beta[training,]) %*% Kb + sum(pmax(out,0)^2) / 2
        iter <- iter + 1
        ...
        if (usegpu) {
          H <- gpuMatrix(K[sv,sv]) + lambda * diag(length(sv))
        }
        else {
          H <- K[sv,sv] + lambda * diag(length(sv))
        }
        
        beta_new <- matrix(0,nrow=dim(Y)[1],ncol=1)
        beta_new[sv,] <- (solve(H) %*% Y[sv,])[]

        step <- beta_new - beta
        
        lsOut <- line_search_nonlinear(step[training,],Kb,beta[dim(beta)[1],],Y,lambda,TRUE)
        t <- lsOut[["t"]]
        Kb <- lsOut[["Kb"]]
        beta <- beta + t * step
  ...
  if (linear) {
    ...
    w <- pslcgOut[["w_l"]]
    out <- pslcgOut[["out_l"]]
    pslOut <- primal_svm_linear(Y,lambda,w,out,usegpu)
    return(pslOut[["alpha"]])
  } 
  else {
    sol <- primal_svm_nonlinear(Y,lambda,usegpu)
    return(sol)
  }
}
